...
CREATE TABLE IF NOT EXISTS ERNÄHRUNGSKATEGORIEN(                              -- Create new table (if not already there): ERNÄHRUNGSKATEGORIEN
  ...
  KATEGORIE     VARCHAR(50),                                                  -- Create new attribute column: KATEGORIE
...
CREATE TABLE IF NOT EXISTS ALLERGENE(                                         -- Create new table (if not already there): ALLERGENE
  ...
  ALLERGEN      VARCHAR(50),                                                  -- Create new attribute column: ALLERGEN
...
CREATE TABLE IF NOT EXISTS REZEPTE(                                           -- Create new table (if not already there): REZEPTE
  ...
  REZEPT        VARCHAR(50),                                                  -- Create new attribute column: REZEPT
...
CREATE TABLE IF NOT EXISTS REZEPTZUTATEN(                                     -- Create new table (if not already there): REZEPTZUTATEN
...
CREATE TABLE IF NOT EXISTS REZEPTKATEGORIEN(                                  -- Create new table (if not already there): REZEPTKATEGORIEN

  REZEPTNR      INTEGER NOT NULL,                                             -- Create a column for a foreign attribute: REZEPTNR
  KATEGORIENR   INTEGER NOT NULL,                                             -- Create a column for a foreign attribute: KATEGORIENR

  FOREIGN KEY (REZEPTNR) REFERENCES REZEPTE (REZEPTNR),                       -- Set REZEPTNR as a foreign key
  FOREIGN KEY (KATEGORIENR) REFERENCES ERNÄHRUNGSKATEGORIEN (KATEGORIENR)     -- Set KATEGORIENR as a foreign key

);

CREATE TABLE IF NOT EXISTS REZEPTALERGENE(                                    -- Create new table (if not already there): REZEPTALERGENE
